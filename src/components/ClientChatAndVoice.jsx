...
// // Renamed component for client-side use
// function ClientChatAndVoice({ onLogout, chatHistoryKey = 'clientVoiceHistory' }) {
...
//   const [userName, setUserName] = useState('User'); // Changed default to 'User'

//   const [showChatHistory, setShowChatHistory] = useState(false);

//   // Mock chat history for client sidebar (changed titles for client context)
//   const [sidebarChats, setSidebarChats] = useState([
//     { id: 'client-chat-1', title: 'Customer Chat' },
//     { id: 'client-chat-2', title: 'Emergency SOP Inquiry' },
//     { id: 'client-chat-3', title: 'General Questions' },
//     { id: 'client-chat-4', title: 'HR Assistance' },
//   ]);
...
//   // Simplified for client, assumes initialSOPData is globally available or fetched differently
//   const getAvailableSOPs = useCallback(() => {
//     return initialSOPData; // Assuming all SOPs are available to client via direct data or pre-loaded
//   }, []);
...
//   // Simplified for client, uses initialSOPData directly
//   const findSOPByKeyword = useCallback((query) => {
//     const lowerQuery = query.toLowerCase();
//     const sops = getAvailableSOPs(); // Uses the client's available SOPs
//     const foundSOPs = sops.filter(sop =>
//       sop.keywords.some(keyword => lowerQuery.includes(keyword.toLowerCase())) ||
//       lowerQuery.includes(sop.title.toLowerCase())
//     );
//     return foundSOPs.length > 0 ? foundSOPs[0] : null;
//   }, [getAvailableSOPs]); // Dependency added
...
//     if (lowerMessage.includes('time')) return `Current time is ${new Date().toLocaleTimeString()}.`;
...
//       const newChatTitle = messageText.length > 20 ? messageText.substring(0, 20) + '...' : messageText;
...
//     }
//   }, [messages, getAIResponse, speakText]);

//   const handleSendChat = () => {
//     if (chatInput.trim()) {
//       handleSendMessage(chatInput.trim(), 'user');
//       setChatInput('');
...
//       { type: 'rain', message: 'Rain soon. Please carry an umbrella!', voice: 'It may rain soon.', probability: 0.15 },
//       { type: 'snow', message: 'Snow expected soon.', voice: 'Snow soon.', probability: 0.05 },
//       { type: 'temp_change', message: 'Sudden temperature change. Stay safe!', voice: 'Temperature changing.', probability: 0.1 },
//       { type: 'severe', message: 'Severe weather! Seek shelter.', voice: 'Severe weather alert!', probability: 0.1 }
//     ];
...
//       if (chatHistoryKey === 'clientVoiceHistory') speakText(greeting); // Use clientVoiceHistory here
//     }
//   }, [chatHistoryKey, userName, speakText]);
...
//       intervalId = setInterval(checkWeatherAndAlert, 25000);
//     } else {
//       clearInterval(intervalId);
//       setCurrentWeatherAlert('Automatic weather alerts off.');
//       setIsWeatherAlertActive(false);
//       window.speechSynthesis.cancel();
//     }
//     return () => {
//       clearInterval(intervalId);
...
//       if (finalTranscript) {
//         setChatInput(finalTranscript); // Put final transcript into input
...
//       // Only speak error if it's not a common 'no-speech' error
...
//     recognition.onend = () => {
//       setIsListening(false);
//       // If there's an interim transcript left, use it as final input if user stopped speaking
//       if (interimTranscript.trim() && !chatInput.trim()) {
//         setChatInput(interimTranscript);
//         handleSendMessage(interimTranscript, 'user');
//       }
//       setInterimTranscript(''); // Ensure interim is cleared
...
//       }
//     };
//   }, [handleSendMessage, speakText, interimTranscript, chatInput]); // Added interimTranscript and chatInput to dependencies
...
//     } else {
//       setChatInput('');
//       setInterimTranscript(''); // Clear interim before starting
...
//             ${theme === 'dark' ? 'bg-gray-800 text-gray-100 hover:bg-gray-700' : 'bg-blue-600 text-white hover:bg-gray-900'
...
//               {/* Voice Alert Icon (Bell) - Using a bell icon */}
//               <svg className={`mr-2 w-5 h-5 ${isWeatherAlertActive ? 'text-yellow-300' : 'text-green-300'}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
//               </svg>
//               <span className="text-sm font-medium">{currentWeatherAlert}</span>
//             </div>
//           </div>
...
//         <div className={`pt-4 mt-4 ${theme === 'dark' ? 'border-t border-gray-700' : 'border-t border-gray-300'}`}>
//           <button
//             onClick={handleLogoutClick}
//             className={`flex items-center bg-red-500 w-full py-2 px-4 rounded-md transition-colors duration-200
//               ${theme === 'dark' ? 'text-white hover:bg-red-700' : 'text-white hover:bg-red-200'}`}
//           >
...
//       {/* Main Content Area */}
//       {/* Removed justify-between. Added h-full. */}
...
//             <div
//               ref={chatAreaRef}
//               // Added min-h-0 for proper flex-grow behavior within a column
...
//                   <div className={`p-3 rounded-xl max-w-[80%] break-words animate-pulse shadow-sm rounded-bl-none
//                     ${theme === 'dark' ? 'bg-gray-600 text-gray-100' : 'bg-gray-200 text-gray-900'}`}>
//                     OzBot is thinking...
//                   </div>
//                 </div>
//               )}
//               {interimTranscript && ( // Display live transcript
...
// export default ClientChatAndVoice;
...
function ClientChatAndVoice({ onLogout, chatHistoryKey = 'adminVoiceHistory' }) {
  ...
  const handleLogoutClick = () => {
    ...
    navigate('/login'); // Navigate back to the admin dashboard
  };
